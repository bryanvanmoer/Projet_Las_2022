...
#define ONE_SECOND_IN_NANOSECOND 1000000
...
// PRE :    arg_path != NULL
//            arg_name != NULL
// POST :    Work like a child and run a "sexecl" (a safe version of the "execl" system call).
void child_sexecl0(void* arg_path, void* arg_name);

// PRE :    arg_path != NULL
//            arg_name != NULL
// POST :    A new child process is created by means of the "fork" function.
//          Firstly, it executes the <sexecl> function with <arg_path> and <arg_name> 
//          as parameters then it exits. 
//            "sexecl" is a safe version of the "execl" system call.
//          The calling process and the child process runs concurrently. 
//          If an error occurs, the calling process is abruptly terminated. 
// RES :     The pid of the chid process.
...
// PRE: fd: a valid file descriptor
//      line: a buffer of at least BUFFER_SIZE+1 chararecters
// POST: fill the buffer with characters read from sting and returns the number of
//       character read.
...
typedef struct StandardStreamFds {
    int fdstdin;
    int fdstderr;
    int oldstdin;
    int oldstderr;
...
// PRE :    standardStream : the path for the file descriptor who replace the stdin.
//            standardErrorStream : the path for the file descriptor who replace the stderr.
// POST :    First <sclose> (a safe version of the "close" system call) the stdin and stderr on screen.
//            Then <sopen()> (a safe version of the "open" system call) the 2 fd (open and trunc it or create it).
//            Then redirect stdin and stderr on their respective fd open.
// RES :    A StandardStreamFds who contains the fd id of stdin and the fd id of stderr.
StandardStreamFds redirectStandardStreams(const char* standardStream, const char* standardErrorStream);

// PRE :    standards != NULL.
// POST :    <sclose> (a safe version of the "close" system call) the 2 fd in standards (fdstdin & fdstderr).
void scloseStandardStreams(const StandardStreamFds standards);

// PRE :	standards != NULL.
//			The fd for stdin and stdout must be close before.
// POST :	redirect stdin and stderr on their respective old fd open.
...
// PRE :	port : the port for the socket
//			maxSimultaneous : the maximum connection in same time.
// POST : 	Create a socket safety.
//			Then setsockopt -> to avoid Address Already in Use.
//			Then bind safety.
//			Then listen safety.
//			On succes, return the file descriptor for the socket.
int initSocketServer(int port, int maxSimultaneous);

// PRE :	ServerIP : the server IP address.
//			Serverport : the port for the socket.
// POST :	Create a socket safety.
//			Then <sconnect> to the socket.
//			On succes, return the file descriptor for the socket.
...
// PRE :	start != NULL
//			end != NULL
// POST :	Subtraction of nano seconds from start and end.
// RES :	The nano seconds between start and end.
